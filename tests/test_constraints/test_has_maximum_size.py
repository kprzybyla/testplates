...
def test_repr(data: st.DataObject, size: int) -> None:
    fmt = "testplates.has_maximum_size(maximum={maximum})"

    maximum = data.draw(st_maximum(size))

    assert (result := has_maximum_size(maximum))

    constraint = unwrap_success(result)
    assert repr(constraint) == fmt.format(maximum=maximum)
...
# noinspection PyTypeChecker
@given(data=st.data(), size=st_size())
def test_failure_when_value_is_above_maximum(data: st.DataObject, size: int) -> None:
...
# noinspection PyTypeChecker
@given(data=st.data())
def test_failure_when_maximum_is_below_zero(data: st.DataObject) -> None:
    below_minimum = data.draw(st_size_below_minimum())

    assert not (result := has_maximum_size(below_minimum))
...
# noinspection PyTypeChecker
@given(data=st.data())
def test_failure_when_maximum_is_above_max_size(data: st.DataObject) -> None:
    above_maximum = data.draw(st_size_above_maximum())

    assert not (result := has_maximum_size(above_maximum))
